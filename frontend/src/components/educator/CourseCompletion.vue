...
export default {
  ...
  data() {
    return {
      localCourses: [],
      nameMapping: {} // Will store mapping from objectId to objectName
    };
  },
  methods: {
    // Fallback method: extract name from URL (last segment, underscores replaced with spaces)
    transformName(url) {
      if (!url) return '';
    ...
    // Use the name mapping if available, otherwise fallback.
    getNameForId(id) {
      return this.nameMapping[id] || this.transformName(id);
    },
    formatPercentage(value) {
      // Display as integer if no fraction, otherwise one decimal place.
    ...
    async fetchData() {
      try {
        ...
        // First, fetch the mapping from activity id to activity name.
        const mappingResponse = await fetch('http://localhost:8000/getNameForActivityId', {
          headers: { Authorization: `Bearer ${authToken}` }
        });
        if (!mappingResponse.ok) {
          throw new Error(`HTTP error from getNameForActivityId! Status: ${mappingResponse.status}`);
        }
        const mappingData = await mappingResponse.json();
        // Build a dictionary from the array of objects.
        if (mappingData && mappingData.objects && Array.isArray(mappingData.objects)) {
          mappingData.objects.forEach(obj => {
            this.nameMapping[obj.objectId] = obj.objectName;
          });
        }
        ...
        // Assume all subcourses share the same parent.
        const parentUrls = Object.values(parentData.parentOfParents);
        const mainCourseUrl = parentUrls.length ? parentUrls[0] : '';
        const mainCourseName = this.getNameForId(mainCourseUrl);
  
        // Fetch the assessment performance data.
        ...
        const subcourses = data.subcourses.map((sub, index) => ({
          ...
          name: this.getNameForId(sub.name),
          ...
          assessments: sub.assessments.map((assess, idx) => ({
            id: idx,
            name: this.getNameForId(assess.activityId),
...
.progress {
  height: 10px;
  background-color: #c40d1e; /* Red color for progress bars */
}
